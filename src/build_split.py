...
Reads manifest.csv, assigns celebrity identities by parsing the id[0-9]+ prefix from 
video_id, and partitions the 59 identities into 41 train / 9 val / 9 test with zero 
overlap.
...
def parse_args():
    ...
    p.add_argument("--stride", type=int, default=60, help="Stride for temporal crop windows")
    p.add_argument("--n-frames", type=int, default=160, help="Window length")
    return p.parse_args()
...
def main():
    ...
    print(f"Manifest OK entries: {len(df[df['status']=='ok'])} | " f"Files on disk: {len(ok)}")

    # Extract identity from video_id
