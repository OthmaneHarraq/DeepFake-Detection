...
def train_and_eval(model_class, dropout, X_tr, y_tr, X_ev, y_ev,
                   lr, wd, seed=42, epochs=30, batch=64):
...
def main():
    ...
    # Extract method from video_id prefix (e.g. AniTalker__id0_0000_... → AniTalker)
    ...
    for method in METHODS:
        ...
        m_train = split_df[(split_df["split"] == "train") &
                           (split_df["class"] == "fake") &
                           (split_df["method"] == method)]
        m_eval  = split_df[split_df["split"].isin(["val", "test"]) &
                           (split_df["class"] == "fake") &
                           (split_df["method"] == method)]
