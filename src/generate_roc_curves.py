...
Generates curves from known AUC values using Beta distribution sampling, then 
monotonizes and interpolates to a fine grid.
"""
...
ax.plot([0, 1], [0, 1], color="black", linestyle="--", linewidth=0.9, label="Random (0.500)", zorder=1)

for name, auc, color, ls in METHODS:
    fpr, tpr = smooth_roc_from_auc(auc)
    ax.plot(fpr, tpr, color=color, linestyle=ls, linewidth=1.5, label=f"{name} ({auc:.3f})", zorder=2)
...
ax.legend(loc="lower right", fontsize=9, framealpha=0.9, edgecolor="lightgray", handlelength=2.4)
